...
bool TerminalOutput::Designate94Charset(
    size_t gsetNumber, const std::pair<wchar_t, wchar_t> charset) {
...
bool TerminalOutput::Designate96Charset(
    size_t gsetNumber, const std::pair<wchar_t, wchar_t> charset) {
  switch (charset.first) {
...
bool TerminalOutput::LockingShiftRight(const size_t gsetNumber) {
  ...
  // If GR is mapped to Latin1, or GR translation is not allowed, we don't need
...
// Routine Description:
// - Returns true if there is an active translation table, indicating that text
// has to come through here Arguments:
// - <none>
// Return Value:
// - True if translation is required.
bool TerminalOutput::NeedToTranslate() const noexcept {
  return !_glTranslationTable.empty() || !_grTranslationTable.empty() ||
         !_ssTranslationTable.empty();
}

void TerminalOutput::EnableGrTranslation(bool enabled) {
  _grTranslationEnabled = enabled;
  // We need to reapply the right locking shift to (de)activate the translation
  // table.
...
bool TerminalOutput::_SetTranslationTable(
    const size_t gsetNumber, const std::wstring_view translationTable) {
  _gsetTranslationTables.at(gsetNumber) = translationTable;
  // We need to reapply the locking shifts in case the underlying G-sets have
  // changed.
