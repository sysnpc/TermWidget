...
namespace PresudoMicrosoft::Console::VirtualTerminal {
  ...
  class CharSet {
   public:
    constexpr CharSet(
        const std::initializer_list<std::pair<wchar_t, wchar_t>> replacements) {
  ...
  template <wchar_t BaseChar, size_t Size>
  constexpr bool operator==(const std::wstring_view lhs,
                            const CharSet<BaseChar, Size>& rhs) {
    return rhs == lhs;
  }
  ...
  static constexpr auto LatinGreek = Latin1BasedCharSet96{
      ...
      {L'\xe0',
       L'\u03b0'},  // Greek Small Letter Upsilon With Dialytika And Tonos
  ...
  static constexpr auto DecSpecialGraphics = AsciiBasedCharSet{
      {L'\x5f', L'\u0020'},  // Blank
      {L'\x60', L'\u2666'},  // Diamond (more commonly U+25C6, but U+2666
                             // renders better for us)
  ...
  static constexpr auto DutchNrcs = AsciiBasedCharSet{
      ...
      {L'\x5b', L'\u0133'},  // Latin Small Ligature Ij (sometimes approximated
                             // as y with diaeresis)
      {L'\x5c', L'\u00bd'},  // Vulgar Fraction One Half
      {L'\x5d', L'\u007c'},  // Vertical Line
      {L'\x7b', L'\u00a8'},  // Diaeresis
      {L'\x7c', L'\u0192'},  // Latin Small Letter F With Hook (sometimes
                             // approximated as f)
  ...
  // Some terminal emulators consider all the French character sets as
  // equivalent, but the 6/6 designator is actually an updated ISO standard,
  // which adds the Micro Sign character, which is not included in the DEC
  // version.
  ...
  static constexpr auto GermanNrcs = AsciiBasedCharSet{
      ...
      {L'\x7b', L'\u00e4'},  // Latin Small Letter A With Diaeresis
      {L'\x7c', L'\u00f6'},  // Latin Small Letter O With Diaeresis
      {L'\x7d', L'\u00fc'},  // Latin Small Letter U With Diaeresis (VT320
                             // manual incorrectly has this as U+00A8)
  ...
  // Some terminal emulators consider all the Nordic character sets as
  // equivalent, but the 6/0 designator is a separate ISO-registered standard,
  // which only maps a subset of the characters included in the DEC version.
  ...
  static constexpr auto SpanishNrcs = AsciiBasedCharSet{
      ...
      {L'\x7b',
       L'\u00b0'},  // Degree Sign (VT320 manual has these last 3 off by 1)
