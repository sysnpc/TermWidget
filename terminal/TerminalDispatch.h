...
#include <string_view>
#include <vector>
#include "../buffer/TextAttribute.h"
#include "../common/Ascii.h"
#include "../common/precomp.h"
#include "../parser/TermDispatch.h"
#include "ITerminal.h"
...
namespace PresudoMicrosoft::Console::VirtualTerminal {
  ...
  class TerminalDispatch : public ITermDispatch {
   public:
    TerminalDispatch(ITerminal* pTerm);

    void Execute(const wchar_t wchControl) override {
      _pTerm->Execute(wchControl);
    }
    ...
    bool CursorHorizontalPositionAbsolute(
        const size_t column) override;                              // HPA, CHA
    ...
    bool CursorPosition(const size_t line,
                        const size_t column) override;     // CUP, HVP
    ...
    bool EraseInDisplay(const EraseType eraseType) override;              // ED
    bool EraseInLine(const EraseType eraseType) override;                 // EL
    ...
    bool SetGraphicsRendition(
        const std::basic_string_view<GraphicsOptions> options) override;  // SGR
    bool DeviceStatusReport(
        const AnsiStatusType statusType) override;    // DSR, DSR-OS, DSR-CPR
    ...
    bool SetPrivateModes(const std::basic_string_view<PrivateModeParams> params)
        override;  // DECSET
    bool ResetPrivateModes(const std::basic_string_view<PrivateModeParams>
                               params) override;                  // DECRST
    bool SetCursorKeysMode(const bool applicationMode) override;  // DECCKM
    bool SetKeypadMode(
        const bool applicationMode) override;  // DECKPAM, DECKPNM
    bool EnableWin32InputMode(
        const bool win32InputMode) override;                // win32-input-mode
    ...
    bool LineFeed(
        const LineFeedType lineFeedType) override;  // IND, NEL, LF, FF, VT
    bool ReverseLineFeed() override;  // RI
    bool SetWindowTitle(
        const std::wstring_view title) override;       // OSCWindowTitle
    ...
    bool SetCursorStyle(
        const DispatchTypes::CursorStyle cursorStyle) override;  // DECSCUSR
    bool SetCursorColor(const COLORREF cursorColor) override;

    bool SetClipboard(
        const std::wstring_view content) noexcept override;  // OSCSetClipboard

    bool SetColorTableEntry(const size_t tableIndex,
                            const DWORD color) override;  // OSCColorTable
    bool SetDefaultForeground(
        const DWORD color) override;  // OSCDefaultForeground
    bool SetDefaultBackground(
        const DWORD color) override;  // OSCDefaultBackground

    bool WindowManipulation(
        const DispatchTypes::WindowManipulationType function,
        const std::basic_string_view<size_t> parameters)
        override;  // DTTERM_WindowManipulation
   ...
   private:
    ...
    struct Offset {
      ...
      bool IsAbsolute;
      // VT origin is at 1,1.
      static constexpr Offset Absolute(const size_t value) {
        return {int(value), true};
      };
      static constexpr Offset Forward(const size_t value) {
        return {int(value), false};
      };
      static constexpr Offset Backward(const size_t value) {
        return {-int(value), false};
      };
      static constexpr Offset Unchanged() { return Forward(0); };
    };

    bool _CursorMovePosition(const Offset rowOffset, const Offset colOffset,
                             const bool clampInMargins) const;
    bool _EraseSingleLineHelper(const ScreenBufferInfo& csbiex,
    ...
    bool _ScrollMovement(const ScrollDirection dir,
    ...
    bool _SetResetPrivateModes(
        const std::basic_string_view<DispatchTypes::PrivateModeParams> params,
        const bool enable);
    bool _PrivateModeParamsHelper(const DispatchTypes::PrivateModeParams param,
                                  const bool enable);
    ...
    size_t _SetRgbColorsHelper(
        const std::basic_string_view<DispatchTypes::GraphicsOptions> options,
        TextAttribute& attr, const bool isForeground) noexcept;
  };
