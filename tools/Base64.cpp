  ...
  if (iter < src.cend())  // Padding char is met.
  {
    ...
    switch (state) {
      ...
      case 2:
        ...
        iter++;  // Skip the padding character and fallthrough to "single
                 // trailing padding character" case.
      case 3:
        while (iter < src.cend()) {
          if (!s_IsSpace(*iter)) {
            return false;
...
// Routine Description:
// - Check if parameter is a base64 whitespace. Only carriage return or line
// feed
//      is valid whitespace.
// Arguments:
