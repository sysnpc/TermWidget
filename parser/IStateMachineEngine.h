...
namespace PresudoMicrosoft::Console::VirtualTerminal {
  class IStateMachineEngine {
   public:
    ...
    virtual bool ActionOscDispatch(const wchar_t wch, const size_t parameter,
                                   const std::wstring_view string) = 0;

    virtual bool ActionSs3Dispatch(
        const wchar_t wch, const std::basic_string_view<size_t> parameters) = 0;
