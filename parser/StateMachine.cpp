...
IStateMachineEngine& StateMachine::Engine() noexcept { return *_engine; }
...
//   This is character sequences less than a space character (null, backspace,
//   new line, etc.) See also
...
//   This is a single-character way to start a control sequence, as opposed to
//   "ESC[".
//
//   Not all single-byte codepages support C1 control codes--in some, the range
//   that would be used for C1 codes are instead used for additional graphic
//   characters.
//
//   However, we do not need to worry about confusion whether a single byte \x9b
//   in a single-byte stream represents a C1 CSI or some other glyph, because by
//   the time we get here, everything is Unicode. Knowing whether a single-byte
//   \x9b represents a single-character C1 CSI or some other glyph is handled by
//   MultiByteToWideChar before we get here (if the stream was not already
//   UTF-16). For instance, in CP_ACP, if a \x9b shows up, it will get converted
//   to \x203a. So, if we get here, and have a \x009b, we know that it
//   unambiguously represents a C1 CSI.
//
// Arguments:
// - wch - Character to check.
// Return Value:
...
//   Intermediates are punctuation type characters that are generally vendor
//   specific and modify the operational mode of a command. See also
...
//   This immediately follows an escape and signifies a varying length control
//   sequence.
// Arguments:
// - wch - Character to check.
// Return Value:
...
// Routine Description:
// - Determines if a character is a delimiter between two parameters in a
// "control sequence"
//   This occurs in the middle of a control sequence after escape and
//   CsiIndicator have been recognized between a series of parameters.
// Arguments:
// - wch - Character to check.
// Return Value:
// - True if it is. False if it isn't.
...
// Routine Description:
// - Determines if a character is a delimiter between two parameters in a
// "operating system control sequence"
//   This occurs in the middle of a control sequence after escape and
//   OscIndicator have been recognized, after the parameter indicating which OSC
//   action to take.
// Arguments:
// - wch - Character to check.
// Return Value:
...
// Routine Description:
// - Determines if a character should be ignored in a operating system control
// sequence Arguments:
// - wch - Character to check.
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isOscInvalid(const wchar_t wch) noexcept {
  return wch <= L'\x17' || wch == L'\x19' || (wch >= L'\x1c' && wch <= L'\x1f');
}

// Routine Description:
// - Determines if a character is "operating system control string" termination
// indicator.
...
// Routine Description:
// - Determines if a character indicates an action that should be taken in the
// ground state -
...
// immediately respond to a C0 control character. Arguments:
...
//   These sequences traditionally start with ESC and a simple letter. No
...
void StateMachine::_ActionEscDispatch(const wchar_t wch) {
  ...
  const bool success = _engine->ActionEscDispatch(
      wch, {_intermediates.data(), _intermediates.size()});
...
// handle a VT52 escape sequence.
//   These sequences start with ESC and a single letter, sometimes followed by
//   parameters.
// Arguments:
// - wch - Character to dispatch.
// Return Value:
...
void StateMachine::_ActionVt52EscDispatch(const wchar_t wch) {
  ...
  const bool success = _engine->ActionVt52EscDispatch(
      wch, {_intermediates.data(), _intermediates.size()},
...
void StateMachine::_ActionCsiDispatch(const wchar_t wch) {
  ...
  const bool success = _engine->ActionCsiDispatch(
      wch, {_intermediates.data(), _intermediates.size()},
      {_parameters.data(), _parameters.size()});
...
// Routine Description:
// - Triggers the Collect action to indicate that the state machine should store
// this character as part of an escape/control sequence. Arguments:
// - wch - Character to dispatch.
// Return Value:
// - <none>
...
// Routine Description:
// - Triggers the Param action to indicate that the state machine should store
// this character as a part of a parameter
...
void StateMachine::_ActionParam(const wchar_t wch) {
  ...
  // If we have no parameters and we're about to add one, get the 0 value ready
  // here.
...
// this character and say nothing. Arguments:
// - wch - Character to dispatch.
// Return Value:
// - <none>
...
// Routine Description:
// - Stores this character as part of the param indicating which OSC action to
// take. Arguments:
...
void StateMachine::_ActionOscDispatch(const wchar_t wch) {
  ...
  const bool success =
      _engine->ActionOscDispatch(wch, _oscParameter, _oscString);
...
void StateMachine::_ActionSs3Dispatch(const wchar_t wch) {
  ...
  const bool success =
      _engine->ActionSs3Dispatch(wch, {_parameters.data(), _parameters.size()});
...
void StateMachine::_EnterGround() noexcept {
  _state = VTStates::Ground;
  _cachedSequence
      .reset();  // entering ground means we've completed the pending sequence
...
//   This state is entered:
//   1. When EscIntermediate characters are seen after an Escape entry (only
//   from the Escape state)
// Arguments:
// - <none>
// Return Value:
// - <none>
...
//   This state is entered:
//   1. When the CsiEntry character is seen after an Escape entry (only from the
//   Escape state)
// Arguments:
// - <none>
// Return Value:
...
//   This state is entered:
//   1. When an invalid character is detected during a CSI sequence indicating
//   we should ignore the whole sequence.
...
//   This state is entered:
//   1. When an intermediate character is seen immediately after entering a
//   control sequence (from CsiEntry)
//   2. When an intermediate character is seen while collecting parameter data
//   (from CsiParam)
// Arguments:
// - <none>
// Return Value:
...
//   This state is entered:
//   1. When an OscEntry character (']') is seen after an Escape entry (only
//   from the Escape state)
// Arguments:
// - <none>
// Return Value:
...
//   This state is entered:
//   1. When the Ss3Entry character is seen after an Escape entry (only from the
//   Escape state)
// Arguments:
// - <none>
// Return Value:
...
//   This state is entered:
//   1. When valid parameter characters are detected on entering a SS3 (from
//   Ss3Entry state)
// Arguments:
// - <none>
// Return Value:
...
//   This state is entered:
//   1. When a VT52 Cursor Address escape is detected, so parameters are
//   expected to follow.
// Arguments:
// - <none>
// Return Value:
...
// Routine Description:
// - Processes a character event into an Action that occurs while in the Ground
// state.
...
// Routine Description:
// - Processes a character event into an Action that occurs while in the Escape
// state.
//   Events in this state will:
//   1. Execute C0 control characters
  ...
  if (_isC0Code(wch)) {
    ...
    if (_isCsiIndicator(wch)) {
      ...
      _EnterOscParam();
               ...
               _engine->ParseControlSequenceAfterSs3()) {
      _EnterSs3Entry();
    } else {
      _ActionEscDispatch(wch);
      _EnterGround();
...
//   2. Ignore Delete characters
//   3. Collect Intermediate characters
//   4. Begin to ignore all remaining parameters when an invalid character is
//   detected (CsiIgnore)
...
void StateMachine::_EventCsiIntermediate(const wchar_t wch) {
  ...
  if (_isC0Code(wch)) {
    ...
    _ActionIgnore();
  } else if (_isCsiParamValue(wch) || _isCsiInvalid(wch) ||
             _isCsiDelimiter(wch) || _isCsiPrivateMarker(wch)) {
    _EnterCsiIgnore();
  } else {
    _ActionCsiDispatch(wch);
    _EnterGround();
...
// Routine Description:
// - Processes a character event into a Action that occurs while in the OscParam
// state.
//   Events in this state will:
...
//   1. Execute C0 control characters
//   2. Ignore Delete characters
//   3. Begin to ignore all remaining parameters when an invalid character is
//   detected (CsiIgnore)
...
//   4. Dispatch a control sequence with parameters for action (always Direct
//   Cursor Address)
// Arguments:
// - wch - Character that triggered the event
// Return Value:
...
// Routine Description:
// - Entry to the state machine. Takes characters one by one and processes them
// according to the state machine rules. Arguments:
...
void StateMachine::ProcessCharacter(const wchar_t wch) {
  ...
  const bool isFromAnywhereChar =
      (wch == AsciiChars::CAN || wch == AsciiChars::SUB);
  ...
  if (isFromAnywhereChar && !(_state == VTStates::Escape &&
                              _engine->DispatchControlCharsFromEscape())) {
    _ActionExecute(wch);
    _EnterGround();
...
bool StateMachine::FlushToTerminal() {
  ...
  if (success && _cachedSequence.has_value()) {
    // Flush the partial sequence to the terminal before we flush the rest of
    // it. We always want to clear the sequence, even if we failed, so we don't
    // accumulate bad state and dump it out elsewhere later.
  ...
  if (success) {
    ...
    //      engine to request the entire sequence get passed through, including
    //      pwchCurr.
...
void StateMachine::ProcessString(const std::wstring_view string) {
  ...
  while (current < string.size()) {
    ...
    if (_processingIndividually) {
      // If we're processing characters individually, send it to the state
      // machine.
      ProcessCharacter(string.at(current));
      ++current;
      if (_state == VTStates::Ground)  // Then check if we're back at ground. If
                                       // we are, the next character (pwchCurr)
      {  //   is the start of the next run of characters that might be
         //   printable.
        _processingIndividually = false;
        start = current;
      }
    } else {
      if (_isActionableFromGround(string.at(
              current)))  // If the current char is the start of an escape
                          // sequence, or should be executed in ground state...
      {
        if (!_run.empty()) {
          ...
          _engine->ActionPrintString(
              allLeadingUpTo);  // ... print all the chars leading up to it as
                                // part of the run...
          _trace.DispatchPrintRunTrace(allLeadingUpTo);
        }

        _processingIndividually =
            true;  // begin processing future characters individually...
        start = current;
        continue;
      } else {
        ++current;  // Otherwise, add this char to the current run to be
                    // printed.
      }
    }
  }

  // When we leave the loop, current has been advanced to the length of the
  // string itself (or one past the array index to the final char) so this
  // `substr` operation doesn't +1 to include the final character (unlike the
  // one inside the top of the loop above.)
  ...
  if (!_processingIndividually && !_run.empty()) {
    ...
    if (_engine->FlushAtEndOfString()) {
      ...
      _cachedSequence =
          _cachedSequence.value_or(std::wstring{}) + std::wstring{_run};
    }
  }
}
...
//     turned off, we don't want any bad state left over for the next input it's
//     turned on for
// Arguments:
// - <none>
// Return Value:
// - <none>
void StateMachine::ResetState() noexcept { _EnterGround(); }

// Routine Description:
// - Takes the given printable character and accumulates it as the new ones
// digit
...
// Arguments:
// - wch - Printable character to accumulate into the value (after conversion to
// number, of course)
// - value - The value to update with the printable character. See example
// above. Return Value:
...
void StateMachine::_AccumulateTo(const wchar_t wch, size_t& value) noexcept {
  ...
  // Values larger than the maximum should be mapped to the largest supported
  // value.
