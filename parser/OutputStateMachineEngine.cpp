#include "OutputStateMachineEngine.h"
...
bool OutputStateMachineEngine::ActionExecute(const wchar_t wch) {
  switch (wch) {
    case AsciiChars::NUL:
      // microsoft/terminal#1825 - VT applications expect to be able to write
      // NUL and have _nothing_ happen. Filter the NULs here, so they don't fill
      // the buffer with empty spaces.
      break;
    case AsciiChars::BEL:
...
// Return Value:
// - true iff we successfully dispatched the sequence.
bool OutputStateMachineEngine::ActionPrintString(
...
// Return Value:
// - true iff we successfully dispatched the sequence.
bool OutputStateMachineEngine::ActionPassThroughString(
    const std::wstring_view string) {
  bool success = true;
  ...
  // If there's not a TTY connection, our previous behavior was to eat the
  // string.
...
// Return Value:
// - true iff we successfully dispatched the sequence.
bool OutputStateMachineEngine::ActionEscDispatch(
    ...
    if (success) {
      switch (designateType) {
        case DesignateCharsetTypes::G0:
          _dispatch->DesignateCharset(wch);
          //          Instance().Log(Codes::DesignateG0);
          break;
        case DesignateCharsetTypes::G1:
          success = false;
          //          Instance().Log(Codes::DesignateG1);
          break;
        case DesignateCharsetTypes::G2:
          success = false;
          //          Instance().Log(Codes::DesignateG2);
          break;
        case DesignateCharsetTypes::G3:
          success = false;
          //          Instance().Log(Codes::DesignateG3);
          break;
        default:
          // If no functions to call, overall dispatch was a failure.
          success = false;
...
// - wch - Character to dispatch.
// - intermediates - Intermediate characters in the sequence
// - parameters - set of numeric parameters collected while parsing the
...
bool OutputStateMachineEngine::ActionCsiDispatch(
  ...
  DispatchTypes::AnsiStatusType deviceStatusType =
      static_cast<DispatchTypes::AnsiStatusType>(
          0);  // there is no default status type.
  size_t repeatCount = 0;
  // This is all the args after the first arg, and the count of args not
  // including the first one.
  const auto remainingParams = parameters.size() > 1
                                   ? parameters.substr(1)
                                   : std::basic_string_view<size_t>{};
  ...
  if (intermediates.empty()) {
    ...
    if (success) {
      switch (wch) {
        case VTActionCodes::CUU_CursorUp:
          _dispatch->CursorUp(distance);
          //          Instance().Log(Codes::CUU);
          break;
        case VTActionCodes::CUD_CursorDown:
          _dispatch->CursorDown(distance);
          //          Instance().Log(Codes::CUD);
          break;
        case VTActionCodes::CUF_CursorForward:
          _dispatch->CursorForward(distance);
          //          Instance().Log(Codes::CUF);
          break;
        case VTActionCodes::CUB_CursorBackward:
          _dispatch->CursorBackward(distance);
          //          Instance().Log(Codes::CUB);
          break;
        case VTActionCodes::CNL_CursorNextLine:
          _dispatch->CursorNextLine(distance);
          //          Instance().Log(Codes::CNL);
          break;
        case VTActionCodes::CPL_CursorPrevLine:
          _dispatch->CursorPrevLine(distance);
          //          Instance().Log(Codes::CPL);
          break;
        case VTActionCodes::CHA_CursorHorizontalAbsolute:
        case VTActionCodes::HPA_HorizontalPositionAbsolute:
          _dispatch->CursorHorizontalPositionAbsolute(distance);
          //          Instance().Log(Codes::CHA);
          break;
        case VTActionCodes::VPA_VerticalLinePositionAbsolute:
          _dispatch->VerticalLinePositionAbsolute(distance);
          //          Instance().Log(Codes::VPA);
          break;
        case VTActionCodes::HPR_HorizontalPositionRelative:
          _dispatch->HorizontalPositionRelative(distance);
          //          Instance().Log(Codes::HPR);
          break;
        case VTActionCodes::VPR_VerticalPositionRelative:
          _dispatch->VerticalPositionRelative(distance);
          //          Instance().Log(Codes::VPR);
          break;
        case VTActionCodes::CUP_CursorPosition:
        case VTActionCodes::HVP_HorizontalVerticalPosition:
          _dispatch->CursorPosition(line, column);
          //          Instance().Log(Codes::CUP);
          break;
        case VTActionCodes::DECSTBM_SetScrollingRegion:
          _dispatch->SetTopBottomScrollingMargins(topMargin, bottomMargin);
          //          Instance().Log(Codes::DECSTBM);
          break;
        case VTActionCodes::ICH_InsertCharacter:
          _dispatch->InsertCharacter(distance);
          //          Instance().Log(Codes::ICH);
          break;
        case VTActionCodes::DCH_DeleteCharacter:
          _dispatch->DeleteCharacter(distance);
          //          Instance().Log(Codes::DCH);
          break;
        case VTActionCodes::ED_EraseDisplay:
          _dispatch->EraseInDisplay(eraseType);
          //          Instance().Log(Codes::ED);
          break;
        case VTActionCodes::EL_EraseLine:
          _dispatch->EraseInLine(eraseType);
          //          Instance().Log(Codes::EL);
          break;
        case VTActionCodes::SGR_SetGraphicsRendition:
          _dispatch->SetGraphicsRendition(
              {graphicsOptions.data(), graphicsOptions.size()});
          //          Instance().Log(Codes::SGR);
          break;
        case VTActionCodes::DSR_DeviceStatusReport:
          _dispatch->DeviceStatusReport(deviceStatusType);
          //          Instance().Log(Codes::DSR);
          break;
        case VTActionCodes::DA_DeviceAttributes:
          _dispatch->DeviceAttributes();
          //          Instance().Log(Codes::DA);
          break;
        case VTActionCodes::SU_ScrollUp:
          _dispatch->ScrollUp(distance);
          //          Instance().Log(Codes::SU);
          break;
        case VTActionCodes::SD_ScrollDown:
          _dispatch->ScrollDown(distance);
          //          Instance().Log(Codes::SD);
          break;
        case VTActionCodes::ANSISYSSC_CursorSave:
          _dispatch->CursorSaveState();
          //          Instance().Log(Codes::ANSISYSSC);
          break;
        case VTActionCodes::ANSISYSRC_CursorRestore:
          _dispatch->CursorRestoreState();
          //          Instance().Log(Codes::ANSISYSRC);
          break;
        case VTActionCodes::IL_InsertLine:
          _dispatch->InsertLine(distance);
          //          Instance().Log(Codes::IL);
          break;
        case VTActionCodes::DL_DeleteLine:
          _dispatch->DeleteLine(distance);
          //          Instance().Log(Codes::DL);
          break;
        case VTActionCodes::CHT_CursorForwardTab:
          _dispatch->ForwardTab(numTabs);
          //          Instance().Log(Codes::CHT);
          break;
        case VTActionCodes::CBT_CursorBackTab:
          _dispatch->BackwardsTab(numTabs);
          //          Instance().Log(Codes::CBT);
          break;
        case VTActionCodes::TBC_TabClear:
          _dispatch->TabClear(clearType);
          //          Instance().Log(Codes::TBC);
          break;
        case VTActionCodes::ECH_EraseCharacters:
          _dispatch->EraseCharacters(distance);
          //          Instance().Log(Codes::ECH);
          break;
        case VTActionCodes::DTTERM_WindowManipulation:
          _dispatch->WindowManipulation(
              static_cast<DispatchTypes::WindowManipulationType>(function),
              remainingParams);
          //          Instance().Log(Codes::DTTERM_WM);
          break;
        case VTActionCodes::REP_RepeatCharacter:
...
// Routine Description:
// - Handles actions that have postfix params on an intermediate '?', such as
// DECTCEM, DECCOLM, ATT610 Arguments:
// - wch - Character to dispatch.
// - parameters - set of numeric parameters collected while parsing the
// sequence. Return Value:
// - True if handled successfully. False otherwise.
bool OutputStateMachineEngine::_IntermediateQuestionMarkDispatch(
    const wchar_t wchAction, const std::basic_string_view<size_t> parameters) {
  bool success = true;
  ...
  if (success) {
    switch (wchAction) {
      case VTActionCodes::DECSET_PrivateModeSet:
        _dispatch->SetPrivateModes(
            {privateModeParams.data(), privateModeParams.size()});
        // TODO: MSFT:6367459 Add specific logging for each of the DECSET/DECRST
        // codes
        _trace.Log(Codes::DECSET);
        break;
      case VTActionCodes::DECRST_PrivateModeReset:
        _dispatch->ResetPrivateModes(
            {privateModeParams.data(), privateModeParams.size()});
        _trace.Log(Codes::DECRST);
        break;
      default:
        // If no functions to call, overall dispatch was a failure.
        success = false;
...
// Return Value:
// - True if handled successfully. False otherwise.
bool OutputStateMachineEngine::_IntermediateExclamationDispatch(
    const wchar_t wchAction) {
  bool success = false;

  switch (wchAction) {
    case VTActionCodes::DECSTR_SoftReset:
      _dispatch->SoftReset();
      //      Instance().Log(Codes::DECSTR);
      break;
    default:
      // If no functions to call, overall dispatch was a failure.
      success = false;
...
// sequence. Return Value:
// - True if handled successfully. False otherwise.
bool OutputStateMachineEngine::_IntermediateSpaceDispatch(
    const wchar_t wchAction, const std::basic_string_view<size_t> parameters) {
  bool success = false;
  ...
  if (success) {
    switch (wchAction) {
      case VTActionCodes::DECSCUSR_SetCursorStyle:
        _dispatch->SetCursorStyle(cursorStyle);
        _trace.Log(Codes::DECSCUSR);
        break;
      default:
        // If no functions to call, overall dispatch was a failure.
        success = false;
...
// Routine Description:
// - Triggers the OscDispatch action to indicate that the listener should handle
// a control sequence.
//   These sequences perform various API-type commands that can include many
//   parameters.
// Arguments:
...
bool OutputStateMachineEngine::ActionOscDispatch(
    const wchar_t /*wch*/, const size_t parameter,
    const std::wstring_view string) {
  bool success = true;
  std::wstring title;
  size_t tableIndex = 0;
  ...
  if (success) {
    switch (parameter) {
      case OscActionCodes::SetIconAndWindowTitle:
        // TODO: add trace log, icon/title
        _dispatch->SetIconAndWindowTitle(title);
        break;
      case OscActionCodes::SetWindowIcon:
        _dispatch->SetIconName(title);
        break;
      case OscActionCodes::SetWindowTitle:
        _dispatch->SetWindowTitle(title);
        _trace.Log(Codes::OSCWT);
        break;
      case OscActionCodes::SetColor:
        _dispatch->SetColorTableEntry(tableIndex, color);
        _trace.Log(Codes::OSCCT);
        break;
      case OscActionCodes::SetForegroundColor:
        _dispatch->SetDefaultForeground(color);
        _trace.Log(Codes::OSCFG);
        break;
      case OscActionCodes::SetBackgroundColor:
        _dispatch->SetDefaultBackground(color);
        _trace.Log(Codes::OSCBG);
        break;
      case OscActionCodes::SetCursorColor:
        _dispatch->SetCursorColor(color);
        _trace.Log(Codes::OSCSCC);
        break;
      case OscActionCodes::ResetCursorColor:
        _dispatch->SetCursorColor(color);
        _trace.Log(Codes::OSCRCC);
        break;
      default:
        // If no functions to call, overall dispatch was a failure.
        success = false;
...
// sequence. Return Value:
// - true iff we successfully dispatched the sequence.
bool OutputStateMachineEngine::ActionSs3Dispatch(
    const wchar_t /*wch*/,
...
// Routine Description:
// - Retrieves the listed graphics options to be applied in order to the "font
// style" of the next characters inserted into the buffer. Arguments:
// - parameters - The parameters to parse
// - options - Space that will be filled with valid options from the
// GraphicsOptions enum Return Value:
// - True if we successfully retrieved an array of valid graphics options from
// the parameters we've stored. False otherwise.
bool OutputStateMachineEngine::_GetGraphicsOptions(
...
// Return Value:
// - True if we successfully pulled an erase type from the parameters we've
// stored. False otherwise.
bool OutputStateMachineEngine::_GetEraseOperation(
    const std::basic_string_view<size_t> parameters,
    DispatchTypes::EraseType& eraseType) const noexcept {
  bool success = false;  // If we have too many parameters or don't know what to
                         // do with the given value, return false.
  ...
  if (parameters.empty()) {
    ...
    const auto param =
        static_cast<DispatchTypes::EraseType>(til::at(parameters, 0));
...
// Routine Description:
// - Retrieves a distance for a cursor operation from the parameter pool stored
// during Param actions. Arguments:
// - parameters - The parameters to parse
// - distance - Receives the distance
// Return Value:
// - True if we successfully pulled the cursor distance from the parameters
// we've stored. False otherwise.
bool OutputStateMachineEngine::_GetCursorDistance(
    const std::basic_string_view<size_t> parameters, size_t& distance) const
    noexcept {
  bool success = false;
...
// Routine Description:
// - Retrieves a distance for a scroll operation from the parameter pool stored
// during Param actions. Arguments:
// - parameters - The parameters to parse
// - distance - Receives the distance
// Return Value:
// - True if we successfully pulled the scroll distance from the parameters
// we've stored. False otherwise.
bool OutputStateMachineEngine::_GetScrollDistance(
    const std::basic_string_view<size_t> parameters, size_t& distance) const
    noexcept {
  bool success = false;
...
// Routine Description:
// - Retrieves a width for the console window from the parameter pool stored
// during Param actions. Arguments:
// - parameters - The parameters to parse
// - consoleWidth - Receives the width
// Return Value:
// - True if we successfully pulled the width from the parameters we've stored.
// False otherwise.
bool OutputStateMachineEngine::_GetConsoleWidth(
    const std::basic_string_view<size_t> parameters, size_t& consoleWidth) const
    noexcept {
  bool success = false;
...
// Routine Description:
// - Retrieves an X/Y coordinate pair for a cursor operation from the parameter
// pool stored during Param actions. Arguments:
// - parameters - The parameters to parse
// - line - Receives the Y/Line/Row position
// - column - Receives the X/Column position
// Return Value:
// - True if we successfully pulled the cursor coordinates from the parameters
// we've stored. False otherwise.
bool OutputStateMachineEngine::_GetXYPosition(
    const std::basic_string_view<size_t> parameters, size_t& line,
  ...
  if (parameters.empty()) {
    ...
    // If there's only one param, leave the default for the column, and retrieve
    // the specified row.
...
// Routine Description:
// - Retrieves a top and bottom pair for setting the margins from the parameter
// pool stored during Param actions Arguments:
// - parameters - The parameters to parse
// - topMargin - Receives the top margin
// - bottomMargin - Receives the bottom margin
// Return Value:
// - True if we successfully pulled the margin settings from the parameters
// we've stored. False otherwise.
bool OutputStateMachineEngine::_GetTopBottomMargins(
    const std::basic_string_view<size_t> parameters, size_t& topMargin,
...
// Return Value:
// - True if we successfully found a device operation in the parameters stored.
// False otherwise.
bool OutputStateMachineEngine::_GetDeviceStatusOperation(
    const std::basic_string_view<size_t> parameters,
  ...
  if (parameters.size() == 1) {
    ...
    switch (param) {
      // This looks kinda silly, but I want the parser to reject (success =
      // false) any status types we haven't put here.
      case (unsigned short)DispatchTypes::AnsiStatusType::OS_OperatingStatus:
        ...
        break;
      case (unsigned short)
          DispatchTypes::AnsiStatusType::CPR_CursorPositionReport:
...
// Arguments:
// - parameters - The parameters to parse
// - privateModes - Space that will be filled with valid params from the
// PrivateModeParams enum Return Value:
// - True if we successfully retrieved an array of private mode params from the
// parameters we've stored. False otherwise.
bool OutputStateMachineEngine::_GetPrivateModeParams(
    const std::basic_string_view<size_t> parameters,
...
bool OutputStateMachineEngine::_VerifyHasNoParameters(
    const std::basic_string_view<size_t> parameters) const noexcept {
  return parameters.empty();
}

// Routine Description:
// - Validates that we received the correct parameter sequence for the Device
// Attributes command.
// - For DA, we should have received either NO parameters or just one 0
// parameter. Anything else is not acceptable. Arguments:
// - parameters - The parameters to parse
// Return Value:
// - True if the DA params were valid. False otherwise.
bool OutputStateMachineEngine::_VerifyDeviceAttributesParams(
    const std::basic_string_view<size_t> parameters) const noexcept {
  bool success = false;
...
// Routine Description:
// - Null terminates, then returns, the string that we've collected as part of
// the OSC string. Arguments:
...
// Routine Description:
// - Retrieves a distance for a tab operation from the parameter pool stored
// during Param actions. Arguments:
// - parameters - The parameters to parse
// - distance - Receives the distance
// Return Value:
// - True if we successfully pulled the tab distance from the parameters we've
// stored. False otherwise.
bool OutputStateMachineEngine::_GetTabDistance(
    const std::basic_string_view<size_t> parameters, size_t& distance) const
    noexcept {
  bool success = false;
...
// Routine Description:
// - Retrieves the type of tab clearing operation from the parameter pool stored
// during Param actions. Arguments:
// - parameters - The parameters to parse
// - clearType - Receives the clear type
// Return Value:
// - True if we successfully pulled the tab clear type from the parameters we've
// stored. False otherwise.
bool OutputStateMachineEngine::_GetTabClearType(
    const std::basic_string_view<size_t> parameters, size_t& clearType) const
    noexcept {
  bool success = false;
...
// Routine Description:
// - Retrieves a designate charset type from the intermediate we've stored.
// False otherwise. Arguments:
// - intermediate - Intermediate character in the sequence
// - designateType - Receives the designate type.
// Return Value:
// - True if we successfully pulled the designate type from the intermediate
// we've stored. False otherwise.
bool OutputStateMachineEngine::_GetDesignateType(
    const wchar_t intermediate, DesignateCharsetTypes& designateType) const
    noexcept {
  bool success = false;
...
// Routine Description:
// - Returns true if the engine should dispatch on the last character of a
// string
...
bool OutputStateMachineEngine::s_ParseColorSpec(const std::wstring_view string,
  ...
  if (foundValidColorSpec) {
    unsigned long color =
        RGB(LOBYTE(colorValues.at(0)), LOBYTE(colorValues.at(1)),
...
bool OutputStateMachineEngine::_GetOscSetColorTable(
    const std::wstring_view string, size_t& tableIndex,
    unsigned long& rgb) const noexcept {
  tableIndex = 0;
  rgb = 0;
...
bool OutputStateMachineEngine::_GetWindowManipulationType(
    const std::basic_string_view<size_t> parameters,
  ...
  if (parameters.size() > 0) {
    switch (til::at(parameters, 0)) {
      ...
      case DispatchTypes::WindowManipulationType::ResizeWindowInCharacters:
        function =
            DispatchTypes::WindowManipulationType::ResizeWindowInCharacters;
        success = true;
        break;
      default:
        success = false;
...
// Return Value:
// - True if we successfully pulled the cursor style from the parameters we've
// stored. False otherwise.
bool OutputStateMachineEngine::_GetCursorStyle(
    const std::basic_string_view<size_t> parameters,
...
// Return Value:
// - <none>
void OutputStateMachineEngine::SetTerminalConnection(
...
//   False otherwise.
bool OutputStateMachineEngine::_GetRepeatCount(
    std::basic_string_view<size_t> parameters, size_t& repeatCount) const
    noexcept {
  bool success = false;
