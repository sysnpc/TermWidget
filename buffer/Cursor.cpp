...
Cursor::Cursor(int x, int y, IRenderTarget* renderTarget) noexcept
...
void Cursor::SetRenderTarget(IRenderTarget* renderTarget) {
  _renderTarget = renderTarget;
}

Cursor::~Cursor() {}

bool Cursor::HasMoved() const noexcept { return _fHasMoved; }

bool Cursor::IsVisible() const noexcept { return _fIsVisible; }

bool Cursor::IsOn() const noexcept { return _fIsOn; }

bool Cursor::IsBlinkingAllowed() const noexcept { return _fBlinkingAllowed; }

bool Cursor::IsDouble() const noexcept { return _fIsDouble; }

bool Cursor::IsConversionArea() const noexcept { return _fIsConversionArea; }

bool Cursor::IsPopupShown() const noexcept { return _fIsPopupShown; }

bool Cursor::GetDelay() const noexcept { return _fDelay; }

int Cursor::GetSize() const noexcept { return _ulSize; }
...
void Cursor::SetDelay(const bool fDelay) noexcept { _fDelay = fDelay; }

void Cursor::SetSize(const int ulSize) noexcept {
  _ulSize = ulSize;
  _RedrawCursor();
}

void Cursor::SetStyle(const int ulSize, const unsigned long color,
...
void Cursor::SetPosition(int NewX, int NewY) noexcept {
  _RedrawCursor();
  _pos.x = NewX;
  _pos.y = NewY;
  _RedrawCursor();
...
void Cursor::_RedrawCursor() noexcept {
  ...
  // (Conversion areas have cursors to mark the insertion point internally, but
  // the user's actual cursor is the one on the primary screen buffer.)
...
// - NOTE: For use with the method that turns the cursor on and off to force a
// refresh
//         and clear the ON cursor from the screen. Not for use with other
//         methods. They should use the other method so refreshes are suppressed
//         while the cursor is off.
// Arguments:
// - <none>
// Return Value:
// - <none>
void Cursor::_RedrawCursorAlways() noexcept {
  if (_renderTarget) _renderTarget->TriggerRedrawCursor(&_pos);
}
...
// - This is primarily to copy properties that would otherwise not be specified
// during CreateInstance
// - NOTE: As of now, this function is specifically used to handle the
// ResizeWithReflow operation.
...
void Cursor::CopyProperties(const Cursor& OtherCursor) noexcept {
  // We shouldn't copy the position as it will be already rearranged by the
  // resize operation.
  ...
  // A resize operation should invalidate the delayed end of line status, so do
  // not copy.
...
Coord Cursor::GetDelayedAtPosition() const noexcept { return _coordDelayedAt; }

bool Cursor::IsDelayedEOLWrap() const noexcept { return _fDelayedEolWrap; }

void Cursor::StartDeferDrawing() noexcept { _fDeferCursorRedraw = true; }
...
CursorType Cursor::GetType() const noexcept { return _cursorType; }

bool Cursor::IsUsingColor() const noexcept {
  return GetColor() != INVALID_COLOR;
}

unsigned long Cursor::GetColor() const noexcept { return _color; }

void Cursor::SetColor(const unsigned int color) noexcept {
  _color = (unsigned long)(color);
}

void Cursor::SetType(const CursorType type) noexcept { _cursorType = type; }
